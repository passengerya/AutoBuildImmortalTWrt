...
- **冲突提示**：冲突组（clashoo↔nikki、advancedplus↔argon、quickfile↔luci-app-run、argon↔aurora↔shadcn）同时启用时同步脚本在生成段顶部输出 ⚠️ 警告；各机型 build 脚本固定加入 Argon，启用 aurora/shadcn 等备选主题（非纯 CSS 主题，会接管 LuCI 菜单/路由）会提示多主题并存
