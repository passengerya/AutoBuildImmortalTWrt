...
> 主题提示：各机型 build 脚本固定加入 Argon 主题；`aurora`/`shadcn` 并非纯 CSS 主题（会接管 LuCI 菜单/路由），如需启用请只保留一个主题界面，同步脚本会在多主题并存时输出 ⚠️ 警告。`luci-theme-aurora` 需配套启用 `luci-app-aurora-config`（极光配置中心）——主题读取配置中心生成的 `/etc/config/aurora` 渲染顶部工具栏，只开主题不开配置中心会排版错乱（同步脚本会输出 ⚠️ 依赖提示）；`luci-i18n-aurora-config-*` 语言包已剔除（烘焙安装会破坏主题渲染）。
