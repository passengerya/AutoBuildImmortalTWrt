...
> 主题提示：各机型 build 脚本固定加入 Argon 主题；`aurora`/`shadcn` 并非纯 CSS 主题（会接管 LuCI 菜单/路由），如需启用请只保留一个主题界面，同步脚本会在多主题并存时输出 ⚠️ 警告。`luci-app-aurora-config`（Aurora 配置中心）已下架（烘焙安装会破坏主题渲染），只保留 `luci-theme-aurora` 主题本身。
